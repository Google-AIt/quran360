...
export const Route = createFileRoute("/walking-quran")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "قصص المتدربين وتجارب المدارس والميسّرين والتحولات السلوكية والمبادرات.",
      },
...
function Page() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 md:py-16">
      <PageHero
        eyebrow="قصص الأثر"
        title="قرآنًا يمشي على الأرض"
        description="أن يتحول القرآن من نص يُقرأ إلى منهج يُعاش، حتى يكون قرآنًا يمشي على الأرض."
      >
        <div className="rounded-2xl border border-primary-foreground/15 bg-primary-foreground/10 p-4 text-sm leading-7 text-primary-foreground/80">
          كل قصة هنا تروي انتقالًا من الفهم إلى ممارسة يمكن رؤيتها وقياس أثرها.
        </div>
      </PageHero>
        ...
        {stories.map((s) => (
          <article key={s.id} className="rounded-3xl border border-border bg-card p-6 shadow-sm">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              {s.person_name} — {s.person_role} · {s.bag_title}
            </p>
          </article>
        ))}
      </div>
