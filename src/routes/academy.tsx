  ...
  head: () => ({
    meta: [
      { title: "أكاديمية القرآن خطوة بخطوة | دورات تطبيق القرآن" },
      {
        name: "description",
        content:
          "دورات إلكترونية مرتبطة بالحقائب القرآنية، كل دورة 5 دروس مع أنشطة وتحديات وتقييم Q360 وشهادة إتمام.",
      },
      { property: "og:title", content: "أكاديمية القرآن خطوة بخطوة" },
      {
        property: "og:description",
        content: "رحلة تدريب متكاملة: محتوى، نشاط، تطبيق، تحدي، تقييم، Q360، شهادة.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 md:py-16">
      <PageHero
        eyebrow="تعلّم، طبّق، تغيّر"
        title="أكاديمية القرآن خطوة بخطوة"
        description={`دورات إلكترونية مرتبطة بالحقائب القرآنية، تنقلك من مشاهدة المحتوى إلى النشاط والتطبيق والتحدي وقياس الأثر. الدورة الواحدة ${data.settings["price_course"]} ريال، وعضوية المتدرب ${data.settings["price_trainee_membership"]} ريال سنويًا وتشمل جميع الدورات.`}
      >
      ...
      <div className="mt-16">
        <SectionTitle
          title="اختر دورتك"
          subtitle="كل دورة هي رحلة قصيرة ومركزة لتحويل معنى قرآني إلى ممارسة يومية."
        />
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          {data.courses.map((c) => (
            <article
              key={c.id}
              className="group rounded-3xl border border-border bg-card p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-soft md:p-8"
            >
      ...
      <p className="mt-10 rounded-2xl bg-secondary/70 p-6 leading-8 text-muted-foreground">
        بقية الدورات تُضاف تباعًا مع إطلاق حقائبها من لوحة الإدارة، وكل دورة تتكون من 5 فيديوهات
        وأنشطة وتحديات عملية ومهام وتقييم وQ360 ونسبة إنجاز وشهادة إتمام.
      </p>
    </div>
  );
}
